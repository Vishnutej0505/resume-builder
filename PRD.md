...
**Resume import ("paste an existing resume") — implemented**: on Home, a "Paste an existing resume instead" / "Import from another resume" link opens a paste box; one AI call extracts the pasted text into the same structured schema (personalInfo + sections + bullets), the user sees a summary of what was found, and confirms before it replaces the master resume (a confirmation step exists specifically because this is destructive and can't be undone). Spends one AI credit for the extraction call itself, gated by the same AI-usage check as everything else. This is what lets Rohit (Persona 2) paste his existing resume rather than retyping it section by section.

**Implemented as "Full Rewrite for This Job"** (ATS Check screen): user pastes a JD, taps Full Rewrite, and every non-empty bullet in the master resume is rewritten with that JD as context in one pass (looping the same per-bullet AI call, not a separate batch API — see `ARCHITECTURE.md` §4.1). The result becomes a session-only "tailored" working copy the user reviews/edits individually before Preview/Export; a visible banner and one-tap "Discard, back to master" make the ephemeral nature obvious rather than surprising. Each bullet actually rewritten spends one AI credit, same accounting as single-bullet rewrite — a full rewrite of a real resume is expected to cost several credits at once, which is itself the natural reason this becomes a paid action once the free pool is spent.
