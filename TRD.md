...
- **Model:** Gemini 2.5 Flash-Lite, called from the Cloudflare Worker (never from the app) via Google's standard REST `generateContent` endpoint — plain `fetch`, no Google SDK needed for one call shape.
